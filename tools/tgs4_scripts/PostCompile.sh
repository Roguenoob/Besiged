...
if ! { [ -x "$has_git" ] && [ -x "$has_cmake" ] && [ -x "$has_gpp" ] && [ -f "/usr/lib/i386-linux-gnu/libmariadb.so.2" ] && [ -f "/usr/lib/i386-linux-gnu/libssl.so" ] && [ -d "/usr/share/doc/g++-6-multilib" ] && [ -d "/usr/include/mysql" ]; }; then
        ...
        if ! [ -x "$has_sudo" ]; then
                ...
                apt-get update
                apt-get install -y git cmake libmariadb-dev:i386 libssl-dev:i386 libmariadb-dev:i386 grep g++ g++-multilib
                ln -s /usr/include/mariadb /usr/include/mysql || true
                rm -rf /var/lib/apt/lists/*
        else
                sudo dpkg --add-architecture i386
                sudo apt-get update
                apt-get install -y git cmake libmariadb-dev:i386 libssl-dev:i386 grep g++ g++-multilib
                sudo ln -s /usr/include/mariadb /usr/include/mysql || true
...
~/.cargo/bin/cargo build --target i686-unknown-linux-gnu --release
...
mkdir -p artifacts
cd artifacts
cmake .. -DCMAKE_CXX_COMPILER=g++ -DCMAKE_CXX_FLAGS=-m32 -DCMAKE_C_FLAGS=-m32 -DMARIA_LIBRARY=/usr/lib/i386-linux-gnu/libmariadb.so.3
make
